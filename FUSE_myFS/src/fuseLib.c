 ...
 *
*/
static int my_read (const char * path, char * buf, size_t size, off_t offset, struct fuse_file_info * fuse){

	int i;

	fprintf(stderr, "--->>>my_readdir: path %s, offset %j\n", path, (intmax_t)offset);
 ...
 * Unlink
 */
static int my_unlink (const char * name){
	int idxDir;

	fprintf(stderr, "--->>>my_open: path %s \n", name);

	//if(findFileByName(path, &idxNodoI)){
	if((idxDir = findFileByName(&myFileSystem, (char *)name + 1)) == -1) {
		return -ENOENT;
	}
